...
\label{sec:prototype}
  ...
  \subsection{Main Features} % (fold)
    ...
    \subsubsection{Tags Overlay} % (fold)
      ...
      The tags overlay (\ref{fig:tags_overlay}) tries to enhance the user's perception on the displayed artists' nodes regarding their tags.
      ...
      These tags are not supported by Spotify's API, and so, Echonest's\footnote{http://developer.echonest.com} API was used.
      More specifically, Echonest calls them \emph{terms}\footnote{http://developer.echonest.com/docs/v4/artist.html\#terms}, that describe the artist.
      They are very similar to the most commonly known \emph{music genres} (like jazz, country, rock), but they also might include more alternative descriptive terms (progressive metal, symphonic, soundtrack).

      The tags are selectable, and so, when selected, the respective artist nodes that are described by those tags, are highlighted (as seen in \ref{fig:tags_overlay}).
